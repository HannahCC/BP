...
public class Metric {
	...
	public static double[] calcMetric(List<Integer> goldLabelList, List<Integer> predictLabelList)
	{
		...
		for(Integer label: classMap.keySet())
		{
			...
			FileTool.writeLog("label: " + label + "\t" + classMap.get(label));
		}
		...
		macroF /= classMap.size();
		FileTool.writeLog("Accuracy: " + accuracy);
		FileTool.writeLog("macro-F: "+ macroF);
		...
		FileTool.writeLog("MAE: " + mae);
		FileTool.writeLog("RMSE: " + rmse);
	...
	{
		ArrayList<String> goldList = FileTool.readFile(goldFile, "utf8");
		ArrayList<String> preList = FileTool.readFile(predictFile, "utf8");
